...
export async function optimisticCreateIdea(
  ...
  await mutate(
    (key) => typeof key === "string" && key.startsWith("/api/ideas?") && key.includes("status=inbox"),
  ...
  try {
    ...
    if (!res.ok) {
      await mutate(
        ...
        (current: { ideas: Idea[] } | undefined) => {
          if (!current) return current
          return { ...current, ideas: current.ideas.filter((i) => i.id !== optimistic.id) }
        },
        { revalidate: false },
      )
      return { ok: false }
    ...
    await mutate(
      ...
      (current: { ideas: Idea[] } | undefined) => {
        ...
        return {
          ...current,
          ideas: current.ideas.map((i) => (i.id === optimistic.id ? real : i)),
        }
      },
      { revalidate: false },
    )
